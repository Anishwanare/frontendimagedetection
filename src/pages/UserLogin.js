...
const UserLogin = () => {
    ...
    if (email && password) {
      try {
        const response = await axios.post(
          `${process.env.REACT_APP_API_BASE_URL}/api/userLogin`,
          loginData
        );
        const { data } = response;
        alert(data.message);
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    if (email && name && password) {
      try {
        const response = await axios.post(
          `${process.env.REACT_APP_API_BASE_URL}/api/userRegister`,
          registerData
        );
        const { data } = response;
        alert(data.message);
            ...
            <div className="rounded-md shadow-sm -space-y-px">
              <div>
                <input
                  ...
                  required
                  className="text-xl rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                />
              </div>
              <div>
                <input
                  ...
                  required
                  className="text-xl rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                />
              </div>
            </div>
